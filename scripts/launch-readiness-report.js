  ...
  try{
    ...
    const by=await pool.query(`
      SELECT subcategory,COUNT(*)::int AS total,
        ...
        COUNT(*) FILTER (WHERE COALESCE(images,'[]'::jsonb) <> '[]'::jsonb)::int AS with_public_image,
        COUNT(*) FILTER (WHERE COALESCE(publish_ready,false)=true)::int AS publish_ready,
        COUNT(*) FILTER (WHERE stock_quantity IS NOT NULL)::int AS inventory_assigned
      FROM sol_products WHERE seed_details->>'catalog_status'='staging_ready'
      GROUP BY subcategory ORDER BY subcategory
    `);
    console.log('[launch-report] catalog',JSON.stringify(q.rows[0]));
    console.log('[launch-report] by_type',JSON.stringify(by.rows));
  }finally{await pool.end();}
}
main().catch(e=>{console.error('[launch-report] failed',e.stack||e.message);process.exit(1);});
